...
export default function App() {
  ...
  // Reset test account balance for marvellousu031@gmail.com
  useEffect(() => {
    if (email === "marvellousu031@gmail.com" && username) {
      // One-time reset per login session for testing zero-balance flow
      const hasReset = sessionStorage.getItem(`reset_${username}`);
      if (!hasReset) {
        ...
        updateDoc(userRef, {
          ...
          sessionStorage.setItem(`reset_${username}`, "true");
          console.log("Test account balance reset to ₦0.00 for testing.");
        }).catch(err => {
