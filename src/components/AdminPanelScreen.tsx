...
import { 
  ...
  AlertCircle, LayoutGrid, Receipt, History
} from "lucide-react";
...
export default function AdminPanelScreen({ onBack, currentAdminUsername }: AdminPanelScreenProps) {
  ...
  const [totalDepositedPool, setTotalDepositedPool] = useState(0);
  const [loading, setLoading] = useState(true);
  const [syncError, setSyncError] = useState<{ query: string; message: string } | null>(null);
  ...
  useEffect(() => {
    ...
    const unsubscribeUsers = onSnapshot(usersRef, (snapshot) => {
      ...
      setSyncError({ query: "Users List", message: error.message });
    ...
    const unsubscribeWithdrawals = onSnapshot(withdrawalsRef, (snapshot) => {
      ...
      setSyncError({ query: "Withdrawals Queue", message: error.message });
      showToast("error", "Failed to sync withdrawals.");
    });

    // Sync Total Successful Deposits (Root Collection)
    // We use the root 'all_deposits' collection which is populated by UpgradeScreen
    // this avoids collectionGroup index requirements and provides reliable real-time updates.
    const depositsRef = collection(db, "all_deposits");
    const unsubscribeDeposits = onSnapshot(depositsRef, (snapshot) => {
      const total = snapshot.docs.reduce((acc, docSnap) => {
        const data = docSnap.data();
        // Sum only successful deposits
      ...
      setTotalDepositedPool(total);
    }, (error) => {
      console.error("Admin deposits subscription error:", error);
      setSyncError({ query: "Deposit Pool (all_deposits)", message: error.message });
    });

    // Backfill: One-time scan of user-level deposits to mirror into root all_deposits
    const performBackfill = async () => {
      try {
        console.log("[Admin] Backfill started: Checking for un-mirrored deposits...");
        const depositsQuery = query(collectionGroup(db, "deposits"), where("status", "in", ["successful", "Success", "completed"]));
        const depositsSnapshot = await getDocs(depositsQuery);
        
        if (depositsSnapshot.empty) {
          console.log("[Admin] Backfill completed: No historical deposits found.");
          return;
        }
        ...
        for (const depDoc of depositsSnapshot.docs) {
          const data = depDoc.data();
          // Use txRef or falling back to doc ID as the unique identifier
          const txRef = data.txRef || data.id || depDoc.id;
          const rootRef = doc(db, "all_deposits", txRef);
          
          const rootSnap = await getDoc(rootRef);
          if (!rootSnap.exists()) {
            // Mirror the record to the root collection
            await setDoc(rootRef, {
              ...data,
              id: txRef,
              txRef: txRef,
              username: data.username || depDoc.ref.parent.parent?.id || "unknown",
              mirroredAt: new Date().toISOString()
            });
            mirroredCount++;
            console.log(`[Admin] Deposit mirrored: ${txRef} (Amount: ₦${data.amount})`);
          } else {
            skippedCount++;
            // Optional: log skip if needed for debugging
            // console.log(`[Admin] Deposit skipped (already mirrored): ${txRef}`);
          }
        }
        
        console.log(`[Admin] Backfill completed: Mirrored ${mirroredCount} new records. ${skippedCount} were already mirrored.`);
      } catch (err) {
        console.warn("[Admin] Backfill notice: CollectionGroup query failed. This is expected if the Firestore index is still building or if there is no historical data yet.", err);
      }
    };

    performBackfill();

    // 4. Sync All Task Submissions (Collection Group)
    ...
    const unsubscribeTaskSubmissions = onSnapshot(taskSubmissionsQuery, (snapshot) => {
      const submissionsList: TaskSubmission[] = snapshot.docs.map(docSnap => ({
        id: docSnap.id,
        ...(docSnap.data() as Omit<TaskSubmission, "id">)
      }));
      setTaskSubmissions(submissionsList);
    }, (error) => {
      console.error("Admin task submissions subscription error:", error);
      setSyncError({ query: "Task Submissions (collectionGroup)", message: error.message });
    });
    ...
    <div className="flex flex-col flex-1 bg-[#fdfaf6] min-h-screen pb-12 overflow-y-auto">
      ...
      <div className="px-6 py-5 bg-gradient-to-r from-[#2b180d] to-[#3d2314] text-white flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[#dfb04d]/25">
        ...
        <div className="flex items-center gap-2.5">
          {syncError ? (
            <span className="flex items-center gap-1 px-2.5 py-1 rounded-full bg-red-500/20 border border-red-500/30 text-red-400 text-[10px] font-bold uppercase tracking-wider">
      ...
      {/* Sync Error Notice */}
      {syncError && (
        <div className="mx-6 mt-6 p-4 bg-red-50 border border-red-200 rounded-2xl flex flex-col gap-2">
          <div className="flex items-center gap-2 text-red-800 font-black text-[11px] uppercase tracking-wider">
            <AlertCircle className="w-4 h-4" />
            Firestore Query Error: {syncError.query}
          </div>
          <p className="text-[10px] text-red-600 font-medium leading-relaxed">
            {syncError.message}
          </p>
          {syncError.message.includes("index") && (
            <p className="text-[9px] text-red-800 font-bold bg-white p-2 rounded-lg border border-red-100">
              💡 This usually means a Firestore Index is missing. Check your browser's inspect console for a link to generate the required index automatically.
            </p>
          )}
        </div>
      )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          <div className="p-5 rounded-2xl bg-white border border-[#8c6239]/15 shadow-sm space-y-2 relative overflow-hidden">
            ...
            <h3 className="text-2xl font-black text-amber-700 font-display">
              {loading ? "..." : totalDepositedPool === 0 ? "₦0" : formatNaira(totalDepositedPool)}
            </h3>
            <p className="text-[10px] text-stone-500 font-medium">
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-4 p-3 bg-white rounded-lg border border-stone-200/50 text-xs">
                      ...
                      <div>
                        ...
                        <p className="font-bold text-[#8c6239] text-[10px] truncate mt-0.5">
                          {req.accountName || "N/A"}
                        </p>
      ...
      {/* Task Submissions Viewer Section */}
      <div className="px-6 mt-8">
        <div className="bg-white rounded-2xl border border-[#8c6239]/15 shadow-sm overflow-hidden flex flex-col">
          <div className="px-5 py-4 border-b border-stone-100 bg-[#fdfaf6]">
            <h3 className="text-xs font-extrabold text-[#3d2314] uppercase tracking-wider font-display">
              Real-Time Task Submissions
            </h3>
            <p className="text-[10px] text-stone-500 font-medium mt-0.5">
              Review task completion metadata and submission history
            </p>
          </div>
            ...
            {taskSubmissions.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {taskSubmissions.map((sub) => (
                  <div 
                    key={sub.id}
                    className="flex flex-col bg-[#fdfaf6] border border-stone-200 rounded-xl overflow-hidden hover:shadow-md transition-all group p-3.5 space-y-2.5"
                  >
                    <div className="flex justify-between items-start">
                      <div className="px-2 py-0.5 bg-amber-100 text-amber-800 rounded text-[8px] font-bold uppercase tracking-widest mb-2 inline-block">
                        {sub.platform || "N/A"}
                      </div>
                    </div>
                    ...
                    <div className="space-y-2.5">
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="text-[10px] font-black text-[#3d2314] truncate">@{sub.username}</p>
                          <p className="text-[9px] text-stone-500 font-bold uppercase tracking-tight">{sub.taskTitle}</p>
                        </div>
                        <span className="text-[9px] font-black text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-100">
                          {formatNaira(sub.rewardAmount)}
                        </span>
                      </div>

                      <div className="grid grid-cols-2 gap-2 pt-2 border-t border-stone-200/50">
                        <div>
                          <span className="text-[8px] text-stone-400 font-bold uppercase block">Plan Tier</span>
                          <p className="text-[9px] font-extrabold text-[#8c6239]">{sub.membershipPlan || "None"}</p>
                        </div>
                        <div className="text-right">
                          <span className="text-[8px] text-stone-400 font-bold uppercase block">Timestamp</span>
                          <p className="text-[9px] font-medium text-stone-600">{formatDate(sub.timestamp)}</p>
                        </div>
                      </div>
                    </div>
                  </div>
