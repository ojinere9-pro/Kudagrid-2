...
export default function AdminPanelScreen({ onBack, currentAdminUsername }: AdminPanelScreenProps) {
  ...
  useEffect(() => {
    ...
    // 3. Sync Total Successful Deposits (Collection Group)
    ...
    const unsubscribeDeposits = onSnapshot(depositsQuery, (snapshot) => {
      ...
      // Note: If this fails, it might be due to missing collection group index in Firestore
      // even if no ordering is applied. Usually simple equality filters are okay.
    });
      ...
      <div className="px-6 mt-8">
        <div className="bg-white rounded-2xl border border-[#8c6239]/15 shadow-sm overflow-hidden flex flex-col">
          <div className="px-5 py-4 border-b border-stone-100 bg-[#fdfaf6]">
            ...
            <p className="text-[10px] text-stone-500 font-medium mt-0.5">
              Review user-uploaded screenshots and task completion metadata
            </p>
          </div>
            ...
            {taskSubmissions.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {taskSubmissions.map((sub) => (
                  <div 
                    key={sub.id}
                    className="flex flex-col bg-[#fdfaf6] border border-stone-200 rounded-xl overflow-hidden hover:shadow-md transition-all group"
                  >
                    {/* Screenshot Preview */}
                    <div className="aspect-video w-full bg-stone-200 relative overflow-hidden group-hover:bg-stone-300 transition-colors">
                      {sub.screenshot ? (
                        <img 
                          src={sub.screenshot} 
                          alt="Task Proof" 
                      ...
                      <div className="absolute top-2 right-2 px-2 py-0.5 bg-black/60 backdrop-blur-sm rounded text-[8px] text-white font-bold uppercase tracking-widest">
                        {sub.platform || "N/A"}
                      </div>
                    </div>
