...
import { doc, getDoc, setDoc, query, collection, where, getDocs, writeBatch, serverTimestamp } from "firebase/firestore";
import { 
    ...
    try {
      if (isLogin) {
        ...
        console.log("Login Attempt Start:", {
          ...
          authProject: auth.app.options.projectId,
        ...
        try {
          ...
          console.error("Login Failed. Firebase Auth Error:", {
            code: authError.code,
            message: authError.message,
            project: auth.app.options.projectId
          });
          // Check for invalid credentials (which includes user-not-found in modern Firebase)
          if (authError.code === 'auth/invalid-credential' || authError.code === 'auth/user-not-found') {
            // Check legacy Firestore logic for migration
            ...
            if (!querySnapshot.empty) {
              ...
              // Verify old plaintext password
              if (oldUserData.password === password) {
                // SUCCESS: User verified against legacy Firestore. Create Auth account.
                userCredential = await createUserWithEmailAndPassword(auth, email.toLowerCase().trim(), password);
                
                // Migrate profile to new UID path
                ...
                delete (migratedData as any).password; // Security: Strip password
                
                await setDoc(newUserRef, migratedData);
                ...
                console.log("Legacy user migrated successfully:", oldUserData.username);
              } else {
        ...
        if (!usernameSnap.empty) throw new Error("Username already exists.");

        // 2. Create Firebase Auth User
        const userCredential = await createUserWithEmailAndPassword(auth, email.toLowerCase().trim(), password);
        
        // 3. Create Firestore User Profile indexed by UID
