        ...
        if (response.status === "successful") {
          ...
          try {
            ...
            await runTransaction(db, async (transaction) => {
              ...
              // Referral Logic: Only on first successful deposit
              if (userData.referredBy && !userData.referralRewardPaid) {
                const referrerRef = doc(db, "users", userData.referredBy);
                const referrerDoc = await transaction.get(referrerRef);

                if (referrerDoc.exists()) {
                  const bonusAmount = depositAmount * 0.5;
                  ...
                  // Create referral transaction record for referrer
                  const refTxId = `REF_BONUS_${username}_${Date.now()}`;
                  const refTxRef = doc(db, "users", userData.referredBy, "referralTransactions", refTxId);
                  ...
                  // Also record globally for admin auditing
                  const globalRefTxRef = doc(db, "referral_rewards", refTxId);
                  transaction.set(globalRefTxRef, {
                    id: refTxId,
