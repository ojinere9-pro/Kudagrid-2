...
export default function UpgradeScreen({ username, depositBalance, currentPlan, email: userEmail, onBack }: UpgradeScreenProps) {
  ...
  const handleDepositSuccess = async (amount: number, reference: string) => {
    console.log(`[Deposit] Initiating Firestore update for ${username}. Amount: ${amount}, Ref: ${reference}`);
    try {
      const userRef = doc(db, "users", username);
      ...
      try {
        ...
        if (userSnap.exists()) {
          ...
          console.log(`[Referral] Raw referrer ID found: ${rawReferrerId}`);
          
          if (rawReferrerId) {
            const cleanReferrerId = rawReferrerId.trim().toLowerCase();
            console.log(`[Referral] Cleaned referrer ID: ${cleanReferrerId}`);
            ...
            if (cleanReferrerId !== username.toLowerCase()) {
              ...
              // Check if a referral commission for this transaction reference already exists
              const referralTxRef = doc(db, "users", cleanReferrerId, "referralTransactions", reference);
              const referralTxSnap = await getDoc(referralTxRef);
              
              if (referralTxSnap.exists()) {
                console.warn(`[Referral] Commission for transaction ${reference} already credited to ${cleanReferrerId}. Skipping.`);
              } else {
                const referrerSnap = await getDoc(referrerRef);
                if (referrerSnap.exists()) {
                  const bonusAmount = amount * 0.5; // 50% commission
                  console.log(`[Referral] Referrer ${cleanReferrerId} found. Calculating 50% commission: ${bonusAmount} NGN (50% of ${amount} NGN)`);
                  
                  // Use a batch or transaction for atomicity
                  await updateDoc(referrerRef, {
                    referralBalance: increment(bonusAmount)
                  });
                  console.log(`[Referral] Successfully updated referralBalance for ${cleanReferrerId} by ${bonusAmount}`);
                  ...
                  console.log(`[Referral] Transaction log created: ${reference}`);
                  console.log(`[Referral] Successfully credited ${bonusAmount} NGN commission to referrer: ${cleanReferrerId}`);
                } else {
                  console.warn(`[Referral] Referrer ${cleanReferrerId} does not exist in Firestore.`);
                }
              }
            } else {
              console.warn(`[Referral] Self-referral detected (${cleanReferrerId}). No commission awarded.`);
            }
          } else {
            console.log(`[Referral] No referrer found for user ${username}.`);
          }
        } else {
          console.error(`[Referral] User document ${username} not found during referral check.`);
        }
      } catch (refErr) {
    ...
    handleFlutterPaymentTrigger({
      callback: (response) => {
        ...
        if (isSuccessful) {
          ...
          handleDepositSuccess(amountToDeposit, response.transaction_id?.toString() || response.tx_ref);
        } else {
