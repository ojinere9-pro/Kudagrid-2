...
import { ArrowLeft, PlusCircle, Check, Award, Sparkles, AlertCircle, Coins, X, History } from "lucide-react";
...
export default function UpgradeScreen({ username, depositBalance, currentPlan, email: userEmail, onBack }: UpgradeScreenProps) {
  ...
  const amountToDeposit = parseFloat(depositAmountInput) || 0;

  const flutterConfig = React.useMemo(() => ({
    public_key: import.meta.env.VITE_FLUTTERWAVE_PUBLIC_KEY || "",
    tx_ref: "KG-" + Date.now() + "-" + Math.floor(Math.random() * 10000),
    amount: amountToDeposit,
    currency: "NGN",
    payment_options: "card,mobilemoney,ussd",
    customer: {
      email: email || `${username}@kudigrid.com`,
      phone_number: "",
      name: username,
    },
    customizations: {
      title: "KudiGrid Wallet Funding",
      description: `Deposit for @${username}`,
      logo: "https://ais-dev-4i4s6qjtpkv7kln62bi7tx-379403444253.europe-west1.run.app/favicon.ico",
    },
  }), [amountToDeposit, email, username]);

  const handleFlutterPaymentTrigger = useFlutterwave(flutterConfig);

  // Debug: Verify key loading (masked)
  useEffect(() => {
    const key = import.meta.env.VITE_FLUTTERWAVE_PUBLIC_KEY;
    if (key) {
      console.log(`[Flutterwave] Key loaded: ${key.slice(0, 8)}...${key.slice(-4)}`);
    } else {
      console.warn("[Flutterwave] Public key is UNDEFINED in environment variables.");
    }
  }, []);
  ...
  const handleDepositSuccess = async (amount: number, txRef: string, transactionId: string) => {
    console.log(`[Deposit] Initiating Firestore update for ${username}. Amount: ${amount}, Ref: ${txRef}`);
    try {
      const userRef = doc(db, "users", username);
      const depositDocRef = doc(db, "users", username, "deposits", txRef);

      // --- Duplicate Prevention ---
      const depositSnap = await getDoc(depositDocRef);
      if (depositSnap.exists()) {
        console.warn(`[Deposit] Transaction ${txRef} already processed. Skipping balance update.`);
        showToast("error", "This transaction has already been processed.");
        setIsDepositModalOpen(false);
        return;
      }

      // 1. Save Deposit Record first to ensure traceability
      ...
      console.log(`[Deposit] Successfully updated depositBalance for ${username} by adding ${amount} NGN`);

      // --- Referral Commission Check & 50% Commission Logic ---
      try {
        console.log(`[Referral] Checking referral data for user: ${username}`);
        const userSnap = await getDoc(userRef);
        if (userSnap.exists()) {
          const userData = userSnap.data();
          const rawReferrerId = userData.referredBy || userData.referral;
          
          if (rawReferrerId) {
            const cleanReferrerId = rawReferrerId.trim().toLowerCase();
            
            // Prevent self-referral bonus
            if (cleanReferrerId !== username.toLowerCase()) {
              const referrerRef = doc(db, "users", cleanReferrerId);
              
              // --- Double Payment Protection for Referrals ---
              const referralTxRef = doc(db, "users", cleanReferrerId, "referralTransactions", txRef);
              const referralTxSnap = await getDoc(referralTxRef);
              
              if (referralTxSnap.exists()) {
                console.warn(`[Referral] Commission for transaction ${txRef} already credited. Skipping.`);
              } else {
        ...
        console.error("[Referral] Commission credit process failed:", refErr);
      }

      showToast("success", `₦${amount.toLocaleString()} added to Deposit Balance! Ref: ${txRef}`);
      setIsDepositModalOpen(false);
      setDepositAmountInput("");
    } catch (err) {
      console.error("Firestore balance upgrade error:", err);
      showToast("error", "Transaction succeeded but failed to update record in database.");
    }
  };

  const handleFlutterPayment = (e: React.FormEvent) => {
    e.preventDefault();

    if (!import.meta.env.VITE_FLUTTERWAVE_PUBLIC_KEY || import.meta.env.VITE_FLUTTERWAVE_PUBLIC_KEY === "YOUR_FLUTTERWAVE_PUBLIC_KEY") {
      showToast("error", "Payment configuration is invalid or missing. Please ensure VITE_FLUTTERWAVE_PUBLIC_KEY is set correctly.");
      return;
    }
    ...
    handleFlutterPaymentTrigger({
      callback: (response) => {
        console.log("[Flutterwave] Full Payment Response:", response);
        
        // Flutterwave V3 statuses can be 'successful' or 'completed' depending on the flow
        const isSuccessful = response.status === "successful" || response.status === "completed";
        
        if (isSuccessful) {
          console.log("[Flutterwave] Payment identified as successful. Crediting wallet...");
          handleDepositSuccess(
            amountToDeposit, 
            response.tx_ref, 
            response.transaction_id?.toString() || response.tx_ref
          );
        } else {
          console.warn("[Flutterwave] Payment status was not successful:", response.status);
          showToast("error", `Payment status: ${response.status || "failed"}. Please contact support if you were debited.`);
        }
        closePaymentModal();
      },
      onClose: () => {
        ...
        {isDepositModalOpen && (
          <div className="fixed inset-0 z-50 flex items-end justify-center">
            ...
            <motion.div
              ...
              className="relative w-full max-w-[400px] max-h-[90vh] bg-white rounded-t-[32px] border-t border-[#d4a017]/30 shadow-2xl z-55 flex flex-col overflow-hidden text-left"
            >
              {/* Header */}
              <div className="flex justify-between items-center p-6 pb-4 border-b border-[#8c6239]/10 bg-white sticky top-0 z-10">
                <div>
                  ...
                  <p className="text-[9px] text-[#8c6239]/70 font-semibold tracking-wider uppercase">Live Flutterwave Integration</p>
                </div>
                <button
                  type="button"
                  onClick={() => setIsDepositModalOpen(false)}
              ...
              <div className="p-6 overflow-y-auto space-y-8 flex-1 custom-scrollbar">
                ...
                <div className="space-y-4">
                  <form onSubmit={handleFlutterPayment} className="space-y-4">
                    <div>
                    ...
                    <div className="p-3.5 bg-[#fdfaf6] rounded-xl border border-[#8c6239]/10 space-y-1.5">
                      ...
                      <div className="flex justify-between items-center text-[10px] font-medium text-[#3d2314]">
                        ...
                        <span className="font-semibold text-emerald-600">Flutterwave (NGN)</span>
                      </div>
                      <div className="flex justify-between items-center text-[10px] font-medium text-[#3d2314]">
                    ...
                    <button
                      type="submit"
                      className="w-full py-4 rounded-xl text-xs font-black uppercase tracking-wider text-white bg-gradient-to-r from-[#3d2314] to-[#2b180d] hover:from-[#2b180d] hover:to-[#1a0f08] border border-[#d4a017]/20 cursor-pointer transition-all active:scale-95 shadow-premium-3d flex items-center justify-center gap-1.5"
                    >
                      <Sparkles className="w-3.5 h-3.5 text-[#d4a017]" />
                      <span>Initiate Flutterwave Pay</span>
                    </button>
                  </form>
                </div>

                {/* History Section */}
                <div className="pt-6 border-t border-[#8c6239]/10 space-y-4">
                  <div className="flex items-center justify-between">
                    <h4 className="text-xs uppercase tracking-widest font-black text-[#3d2314] flex items-center gap-2">
                  ...
                  <DepositHistory username={username} />
                </div>
              </div>
            </motion.div>
          </div>
