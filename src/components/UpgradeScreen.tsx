...
export default function UpgradeScreen({ username, depositBalance, currentPlan, email: userEmail, onBack }: UpgradeScreenProps) {
  ...
  const handleDepositSuccess = async (amount: number, txRef: string, transactionId: string) => {
    console.log(`[Deposit Trace] Starting atomic wallet credit. User: ${username}, Amount: ₦${amount}, Ref: ${txRef}`);
    setLoadingPlanId("DEPOSIT_PENDING"); 
    ...
    try {
      ...
      // 1. Root collection update (Admin Panel)
      const rootDepositRef = doc(db, "all_deposits", txRef);
      batch.set(rootDepositRef, {
        id: txRef,
      ...
      // 2. User history update
      const userDepositRef = doc(db, "users", cleanUsername, "deposits", txRef);
      batch.set(userDepositRef, {
        id: txRef,
        amount: Number(amount),
        transactionId: transactionId,
        txRef: txRef,
      ...
      console.log(`[Deposit Trace] Committing batch for ${txRef}...`);
      await batch.commit();
      console.log(`[Deposit Trace] SUCCESS: Wallet credited for ${txRef}`);

      showToast("success", `₦${amount.toLocaleString()} added to your balance!`);
      
      setIsDepositModalOpen(false);
      setDepositAmountInput("");

    } catch (err) {
      console.error("[Deposit Trace] BATCH FAILURE:", err);
      const msg = err instanceof Error ? err.message : "Internal Error";
      showToast("error", `Credit failed: ${msg}. Please contact support.`);
    } finally {
  ...
  const handleFlutterPayment = (e: React.FormEvent) => {
    ...
    if (!publicKey || publicKey === "YOUR_FLUTTERWAVE_PUBLIC_KEY") {
      showToast("error", "Flutterwave Public Key is not configured.");
      return;
    }
    ...
    console.log(`[Flutterwave] Opening modal for ${username}, Amount: ₦${amountToDeposit}`);
    
    handleFlutterPaymentTrigger({
      callback: (response) => {
        console.log("[Flutterwave] Callback fired. Response:", response);
        
        // Ensure modal is closed quickly
        closePaymentModal();

        const isSuccessful = response.status === "successful" || response.status === "completed";
        
        if (isSuccessful) {
          const finalAmount = response.amount || amountToDeposit;
          const finalTxRef = response.tx_ref || `KG-AUTO-${Date.now()}`;
          const finalId = response.transaction_id?.toString() || finalTxRef;

          console.log(`[Flutterwave] Success confirmed. Triggering credit for ₦${finalAmount}`);
          // Run the async credit process
          handleDepositSuccess(finalAmount, finalTxRef, finalId);
        } else {
          console.warn("[Flutterwave] Status unsuccessful:", response.status);
          showToast("error", `Payment was ${response.status || "unsuccessful"}.`);
        }
      },
      onClose: () => {
