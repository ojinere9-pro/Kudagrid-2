...
import { doc, getDoc, updateDoc, increment, collection, setDoc, writeBatch } from "firebase/firestore";
...
export default function UpgradeScreen({ username, depositBalance, currentPlan, email: userEmail, onBack }: UpgradeScreenProps) {
  ...
  const handleDepositSuccess = async (amount: number, txRef: string, transactionId: string) => {
    setLoadingPlanId("DEPOSIT_PENDING"); // Reuse loading state for UI feedback
    
    try {
      // Update all_deposits directly from frontend for immediate reflection in Admin Panel
      // This uses the same kudigrid project ID as the rest of the frontend
      const rootDepositRef = doc(db, "all_deposits", txRef);
      await setDoc(rootDepositRef, {
        id: txRef,
        username: username,
      ...
      if (response.ok && result.success) {
        if (response.status === 409 || result.alreadyProcessed) {
          console.log(`[Deposit] Transaction ${txRef} already processed (Status 409).`);
          showToast("success", "This transaction was already processed and credited.");
        } else {
          console.log(`[Deposit] Backend verification successful for ref: ${txRef}`);
          showToast("success", `₦${amount.toLocaleString()} verified and added to your balance!`);
        }
        setIsDepositModalOpen(false);
        setDepositAmountInput("");
      } else {
        console.error("[Deposit] Backend verification failed:", result.error);
        showToast("error", result.error || "Verification failed. Please contact support.");
      }
    } catch (err) {
      console.error("[Deposit] Network error during verification:", err);
